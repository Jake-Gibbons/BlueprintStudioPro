...
struct FloorPlanView: View {
            ...
            ZStack {
                ...
                ForEach(floorPlan.rooms) { room in
                    ...
                    RoomRender(
                        ...
                        externalWidthPt: settings.externalWallWidthPt,
                        internalWidthPt: settings.internalWallWidthPt,
                        roomFillOpacity: settings.roomFillOpacity
                    )
                    ...
                    // Stairs (non-interactive)
                    stairs(for: room, size: geometry.size, effectiveScale: effectiveScale, effectiveOffset: effectiveOffset)
                        .allowsHitTesting(false)
                        .zIndex(1)
    ...
    private func mainGesture(size: CGSize) -> some Gesture {
        DragGesture(minimumDistance: 0)
            .onChanged { value in
                ...
                case .addStairs:
                    // Find the target room under the tap (prefer active)
                    var targetRoom: Room?
                    if let selID = floorPlan.activeRoomID,
                       let sr = floorPlan.rooms.first(where: { $0.id == selID }),
                       sr.contains(point: modelPoint) {
                    ...
                    if let room = targetRoom {
                        floorPlan.addStairs(in: room.id, at: modelPoint)
    ...
    private struct RoomRender: View {
        ...
        let externalWidthPt: CGFloat
        let internalWidthPt: CGFloat
        let roomFillOpacity: Double
        
        var body: some View {
            let path = room.path(using: transform)
            ZStack {
                path.fill(room.fillColor.opacity(roomFillOpacity))
    ...
    private func stairs(for room: Room, size: CGSize, effectiveScale: CGFloat, effectiveOffset: CGSize) -> some View {
        ZStack {
            ForEach(room.stairs) { s in
                // model rect centered at s.center
                let halfL = s.length / 2
                let halfW = s.width  / 2
                let p0 = CGPoint(x: s.center.x - halfL, y: s.center.y - halfW)
                let rectModel = CGRect(x: p0.x, y: p0.y, width: s.length, height: s.width)
                let centerScreen = modelToScreen(s.center, size: size, scale: effectiveScale, offset: effectiveOffset)
                
                // Build step lines in model space (unrotated), then rotate by s.rotation
                let stepCount = max(1, s.steps)
                let stepSpacing = s.length / CGFloat(stepCount)
                
                let sRectScreen = modelRectToScreen(rectModel, size: size, scale: effectiveScale, offset: effectiveOffset)
                StairsShape(rectModel: .zero, steps: stepCount)
                    .stroke(Color.gray.opacity(0.9), lineWidth: 2)
                    .background(
                        StairsShape(rectModel: .zero, steps: stepCount)
                            .fill(Color.gray.opacity(0.12))
                    )
                    .frame(width: sRectScreen.width, height: sRectScreen.height)
                    .position(centerScreen)
                    .rotationEffect(Angle(radians: s.rotation))
    ...
    /// Simple stairs geometry (unrotated), drawn in SCREEN space inside a rotated container
    private struct StairsShape: Shape {
        let rectModel: CGRect
        let steps: Int
        
        func path(in rect: CGRect) -> Path {
            var p = Path()
            // We cannot convert model->screen here; instead, we draw unit coords and rely on parent scaling.
            // We'll approximate: use 'rect' given by SwiftUI (already sized by parent).
            // Draw the outer rect
            p.addRect(rect)
            // Steps as lines across the short side
            if steps > 1 {
                let spacing = rect.width / CGFloat(steps)
                for i in 1..<steps {
                    let x = rect.minX + spacing * CGFloat(i)
