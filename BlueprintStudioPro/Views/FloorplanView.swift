...
// MARK: - File-scope snapping helpers so nested views can use them
...
/// Two-finger pan, pinch zoom, move rooms, resize walls (Resize mode or single-wall),
/// live openings preview, editable dimensions (meters), grid snapping,
/// and toggle to show/hide dimensions.
struct FloorPlanView: View {
    ...
    // Working vertices for custom polygon (Draw Wall tool)
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            ZStack {
                ...
                // Rooms and attachments (active floor)
                ForEach(floorPlan.rooms) { room in
                    ...
                    // Base fill / outline
                    room.path(using: { modelToScreen($0, size: geometry.size, scale: effectiveScale, offset: effectiveOffset) })
                        .fill(room.fillColor)
                        .overlay(
                            isResizeMode ?
                                room.path(using: { modelToScreen($0, size: geometry.size, scale: effectiveScale, offset: effectiveOffset) })
                        ...
                        // Move room (Select tool only)
                        .simultaneousGesture(
                            DragGesture(minimumDistance: 5)
                                .onChanged { value in
                                    guard currentTool == .select,
                                          floorPlan.selectedRoomID == room.id,
                                          let binding = bindingForRoom(id: room.id) else { return }
                                ...
                                .onEnded { _ in
                                    if snapToGrid, let binding = bindingForRoom(id: room.id) {
                                        var updated = binding.wrappedValue
                                        for i in updated.vertices.indices {
                    ...
                    // === RESIZE MODE: show ALL wall highlights + handles ===
                    if isResizeMode {
                        ...
                        ForEach(0..<count, id: \.self) { wall in
                            ...
                            // FAT invisible hit-rail over the wall for drag-resize
                            WallHitRail(
                                roomID: room.id,
                                wallIndex: wall,
                                v1: v1, v2: v2,
                            ...
                            // Handle circle offset from mid along normal
                    ...
                    // === Single-wall selection mode (legacy) ===
                    ...
                    // Dimensions (subtle) + editable labels
                    if showDimensions {
                ...
                // Two-finger pan overlay (covers entire canvas) — only intercepts when 2+ fingers
            ...
            .simultaneousGesture(drawingGesture(size: geometry.size))
        }
    }
    ...
    private func drawingGesture(size: CGSize) -> some Gesture {
        DragGesture(minimumDistance: 0)
            .onChanged { value in
                ...
                let effectiveOffset = CGSize(width: panOffset.width + liveTwoFingerPan.width,
                                             height: panOffset.height + liveTwoFingerPan.height)
                let modelPoint = screenToModel(value.location, size: size, scale: effectiveScale, offset: effectiveOffset)
            ...
            .onEnded { value in
                ...
                case .select:
                    ...
                    if let _ = floorPlan.selectedRoomID {
                ...
                case .drawRoom:
                    floorPlan.addSquareRoom(center: modelPoint)
                case .drawWall:
                    if let first = workingVertices.first {
                        let firstScreen = CGPoint(x: first.x * effectiveScale + size.width/2 + effectiveOffset.width,
                    ...
                    workingVertices.append(modelPoint)
                case .addWindow, .addDoor:
                    if let preview = previewOpening {
    ...
    // MARK: - Dimensions (with editable labels)
...
/// A wide invisible rail centered on a wall that captures drag
/// and moves the wall along its normal based on absolute finger movement.
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            Path { p in p.move(to: sStart); p.addLine(to: sEnd) }
                .stroke(Color.clear, lineWidth: 36) // fat rail
                .contentShape(Rectangle())
                .gesture(
                    DragGesture(minimumDistance: 0)
                        .onChanged { value in
                            ...
                            // Absolute delta from drag start, in MODEL space
...
/// A visible circular handle near the wall that delegates to the same resizing logic.
...
// MARK: - EditableDimensionField (missing helper view)

/// Small, rotated TextField used to edit a dimension label.
